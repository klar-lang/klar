...
class ShortenRequest {
  ...
  static fromJSON(data) {
    const errors = [];
    if (data.url === undefined || data.url === null) errors.push({ field: 'url', error: 'required' });
    if (data.url && data.url.length < 1) errors.push({ field: 'url', error: 'min_len(1)' });
    if (errors.length > 0) return { ok: false, errors };
    return { ok: true, value: new ShortenRequest(data.url) };
  }
  static schema() {
    return {
      name: 'ShortenRequest',
      fields: {
        url: { type: 'String', validations: ['min_len(1)'] },
      },
    };
  }
}
...
class ShortUrl {
  constructor(code, url, created_at) {
    this.code = code;
    this.url = url;
    this.created_at = created_at;
  }
  toJSON() {
    return { code: this.code, url: this.url, created_at: this.created_at };
  }
  static fromJSON(data) {
    const errors = [];
    if (data.code === undefined || data.code === null) errors.push({ field: 'code', error: 'required' });
    if (data.url === undefined || data.url === null) errors.push({ field: 'url', error: 'required' });
    if (data.created_at === undefined || data.created_at === null) errors.push({ field: 'created_at', error: 'required' });
    if (errors.length > 0) return { ok: false, errors };
    return { ok: true, value: new ShortUrl(data.code, data.url, data.created_at) };
  }
  static schema() {
    return {
      name: 'ShortUrl',
      fields: {
        code: { type: 'String', validations: [] },
        url: { type: 'String', validations: [] },
        created_at: { type: 'String', validations: [] },
      },
    };
  }
}
...
function shorten(req) {
  const body = req.json();
  const url = body.url;
  const code = string.slice(crypto.uuid(), 0, 8);
  const short = new ShortUrl(code, url, time.now());
  return Response.json(short);
}

function redirect(req) {
  const code = req.param("code");
  return Response.text("redirect placeholder");
}
...
(function test_shorten_creates_code() {
  const req = Request.post("/shorten", new ShortenRequest("https://example.com"));
  const res = shorten(req);
  assert;
  return (res.status === 200);
  console.log('  \x1b[32m✓\x1b[0m shorten_creates_code');
})();
